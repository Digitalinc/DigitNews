...
class dn_Controller_layer {
    ...
    function parse_sources_feed() {
        ...
        for ($index = 0; $index < mysqli_num_rows($JSONSources); $index++) {
            ...
            if ($tempsource[2] == "top,latest") {
                $this->parse_feed($tempsource[0], $tempsource[1], "latest", $api_key);
            } else {
                $this->parse_feed($tempsource[0], $tempsource[1], "top", $api_key);
            }
        }
    }

    function parse_sources_woapi_feed($sid) {
        $wofeedSources = $this->parse_sources_woapi_sid($sid);
        $retmainval = "";
        for ($index = 0; $index < mysqli_num_rows($wofeedSources); $index++) {
            $tempsource = mysqli_fetch_row($wofeedSources);
            try {
                $this->parse_feed_woapi($tempsource[0], $tempsource[1], $tempsource[2]);
            } catch (Exception $exc) {
                //$retmainval = $retmainval . "$tempsource[0] - failed \n";
                $obj->parse_Feed_Dashboard_Status($source_id,$source_unique_id, 0, 0);
            }
        }
        //return $retmainval;
    }
    ...
    function parse_feed($source_uniqueid, $source_id, $sort, $api_key) {
        ...
        $obj->parse_Feed_Dashboard_Status($source_id,$source_unique_id, $successcount, $failurecount);
    ...
    function parse_feed_woapi($source_unique_id, $sourceid, $urllink) {
        ...
        $parsefeed_obj->parse_Feed_Dashboard_Status($sourceid,$source_unique_id, $successcount, $failurecount);
    ...
    function parse_sources_woapi_sid($sid)
    {
        $Sourceobjsid = new dn_model_layer();
        $tempSources = $Sourceobjsid->getFeedSources_WoAPI_Sid($sid);
        return $tempSources;
    }
    function parse_sources_woapi_minmax()
    {
        $Sourceobjminmax = new dn_model_layer();
        $tempSources = $Sourceobjminmax->getFeedSources_WoAPI_MinMax();
        $rest= mysqli_fetch_row($tempSources);
    ...
    {
        $dashobj=new dn_model_layer();
        $result= $dashobj->feed_Dashboard_Return_Data();
        $htmldata="";
        for ($index = 0; $index < mysqli_num_rows($result); $index++) {
            $rt= mysqli_fetch_row($result);
            $htmldata=$htmldata.$rt[0]." - ".$rt[1]." - ".$rt[2]." - ".$rt[3]." - ".$rt[4]." - ".$rt[5]."<br />";
        }
        echo $htmldata;
    }
